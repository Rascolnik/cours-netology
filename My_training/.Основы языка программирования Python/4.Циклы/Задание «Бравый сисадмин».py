"""
у нас есть список в который помещены строки
нужно взять стоку и найти в ней соответствие предложенному списку
так же нужно взять индекс списка model и сравнить его со списком available
"""


def solve(models: list, available: list, manufacturers: list):
    repair_count = 0  # количество покупок
    ssds = []  # список покупок
    # код вашего решения ниже:
    for i in range(len(models)):
        if available[i] == 1:        # если диск в наличии
            model = models[i]       # берем 1 строку из списка по индексу
            for manufacturer in manufacturers:      # берем производителей по очереди
                if manufacturer in model:       # ищем совпадение в строке из списка доступных
                    ssds.append(model)      # помещаем название в итоговый список покупок
                    repair_count += 1       # количество купленных дисков
                    break  # уже нашли подходящего производителя
